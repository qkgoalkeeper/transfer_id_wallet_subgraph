...
contract GravatarRegistry {
    event NewGravatar(address sender, address receiver, uint256 money);
  ...
  function createGravatar(address _sender, address _receiver,uint256 _money) public {
    // require(ownerToGravatar[msg.sender] == 0);
    gravatars.push(Gravatar(_sender, _receiver, _money));
    // gravatarToOwner[id] = msg.sender;
    // ownerToGravatar[msg.sender] = id;
    for (uint i = 1; i <= 100; i++) {
          emit NewGravatar(_sender,_receiver,_money);
    }
